...
let bbMemo = {
  ...
  .bb-cont blockquote::before{position:absolute;top:5px;left:10px;content:'鈥�';font-weight:700;font-size:28px;font-family:Georgia,serif;line-height:2rem;}
  ...
  let load = '<div class="bb-load"><button class="load-btn button-load">鍔犺浇涓€︹€�</button></div>'
  ...
  async function updateHTMl(data){
    ...
    for(let i=0;i < data.length;i++){
        ...
        let datacountDOM = ""
        if(bbMemo.twiEnv){
          datacountDOM = `<div class="datacount" data-twienv="${bbMemo.twiEnv}" data-id="${bbID}" onclick="loadTwikoo(this)"> ${data[i].count} 鏉¤瘎璁� </div>`
        }
    ...
    if(document.querySelector('button.button-load')) document.querySelector('button.button-load').textContent = '鍔犺浇鏇村';
  ...
  //鎼滅储 Memo 锛屽熀浜� v1 api锛岄渶鎵嬪姩娣诲姞 html 濡傦細<span onclick="serchMemo()">鎼滅储</span>
  function serchMemo(){
    let serchText = prompt('鎼滅偣鍟ワ紵','');
  ...
  //鏄剧ず鏍囩鍒楄〃锛岄渶閰嶅悎 cloudflare worker 椋熺敤
